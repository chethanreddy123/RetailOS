...
export default function EditCustomerModal({ customer, open, onOpenChange, onSaved }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="text-[14px] font-semibold">Edit Customer</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3 pt-1">
          <div className="space-y-1">
            <p className="text-[11px] font-medium text-[#BBBBBB]">Name</p>
            <input className={fieldCls} value={name} onChange={e => setName(e.target.value)} required />
          </div>
          <div className="space-y-1">
            <p className="text-[11px] font-medium text-[#BBBBBB]">Phone (10 digits)</p>
            <input className={fieldCls} value={phone} onChange={e => setPhone(e.target.value.replace(/\D/g, '').slice(0, 10))} required maxLength={10} />
          </div>
          <div className="space-y-1">
            <p className="text-[11px] font-medium text-[#BBBBBB]">Age (optional)</p>
